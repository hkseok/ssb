...
# from get_item import get_item 으로 사용해도 된다.
...
class collector():
    ...
    def db_setting(self):
        ...
        self.engine_bot = create_engine("mysql+mysqldb://root:" + "seok6533" + "@localhost:3306/" + db_name,
                                        encoding='utf-8')
    ...
    def test(self):
        ...
        total_data = self.api.get_total_data('005930', '20200916')
        print("total_data: !!!")
        print(total_data)

        # get_one_day_option_data : openapi클래스의 메서드
        one_data = self.api.get_one_day_option_data('005930', '20200916', 'close')
        print("one_data: !!!")
        print(one_data)
...
#  __name__ ? => 현재 모듈의 이름을 담고 있는 내장 변수, 우리는 colector 라는 파일을 실행한다! -> 이 파일의 이름은 __main__ 이 된다.
# import openapi 를 통해서 openapi 소스코드를 참고를 하는 경우 openapi 파일의 __name__은 "openapi"로 찍힌다.
...
if __name__ == "__main__":
    ...
    # 아래는 키움증권 openapi 를 사용하기 위해 사용하는 한 줄! 이해 할 필요 X
    app = QApplication(sys.argv)
    # c = collector() 이렇게 c라는 collector 라는 클래스의 인스턴스를 만든다.
