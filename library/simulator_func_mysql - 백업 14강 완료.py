...
class simulator_func_mysql:
    ...
    def variable_setting(self):
        ...
        if self.simul_num == 1:
            ...
            self.total_transaction_price = 10000000000  # 거래대금 100억 이상
            self.interval_month = 3
            self.vol_mul = 5  # 거래량 전일 대비 500% 증가
            self.d1_diff_rate = 2  # 전일 대비 2% 이상 상승률
    ...
    def db_to_realtime_daily_buy_list(self, date_rows_today, date_rows_yesterday, i):
        ...
        # 5 / 20 골든 and 투자위험, 투자주의 종목 걸래내서 buy
        ...
        # 120이평 골든, 거래대금 100억 이상, 거래량 5일평균 대비 500% 이상 증가
        elif self.db_to_realtime_daily_buy_list_num == 5:
            sql = "select * from `" + date_rows_yesterday + "` a " \
                    "where yes_clo120 > yes_clo5 and yes_clo5 > clo120 " \
                    "and volume * close > '%s' " \
                    "and vol5 * '%s' < volume " \
                    "and d1_diff_rate > '%s' " \
                    "and NOT exists (select null from stock_konex b where a.code=b.code)" \
                    ...
                    "and NOT exists (select null from stock_invest_danger g where a.code=g.code and g.post_date <= DATE('%s') and (g.cleared_date > DATE('%s') or g.cleared_date is null) group by g.code)" \
                    "and a.close < '%s' " \
                    "order by volume * close desc"  # 거래대금 ( volume * close ) 의 역순으로 정렬해서 매수 한다.

            realtime_daily_buy_list = self.engine_daily_buy_list.execute(sql % ( date_rows_yesterday, self.total_transaction_price, self.vol_mul, self.d1_diff_rate, self.interval_month, date_rows_yesterday, date_rows_yesterday, date_rows_yesterday, date_rows_yesterday, date_rows_yesterday, self.invest_unit)).fetchall()
        ...
        # 절대 모멘텀 전략 : 특정일 전의 종가 보다 n% 이상 상승한 종목 매수 (query version)
